...
void MainWindow::needleCalibration()
{
	  std::cout<<"Needle Calibration"<<std::endl;

	  //NeedleCalibrationWidget* needleCalibration = new NeedleCalibrationWidget();
  ...
  //required by IGSTK for the time stamping used by the tracker
	...
	//  QString qtNeedleFilename = QFileDialog::getOpenFileName(this, tr("Open Needle Rom"),
    //    QDir::currentPath(),tr("Rom Files (*.rom)"));
	//  std::string needleFilename = std::string(qtNeedleFilename.toAscii().data());

	//  needleCalibration->setRomFile(needleFilename);
    //  needleCalibration->show();
...
void MainWindow::checkNeedleCalibration()
{
